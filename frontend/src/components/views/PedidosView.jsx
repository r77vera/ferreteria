...
const PedidosView = () => {
  ...
  const fetchPedidos = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await getPedidos(currentPage, pedidosPerPage, fechaInicio || null, fechaFin || null);
  ...
  return (
    <div className="users-view-container">
      ...
      {isLoading ? (
        ...
        <>
          {pedidos.length === 0 ? (
          <p className="no-results-message">No se encontraron pedidos para el rango de fechas seleccionado.</p>
        ) : (
          <>
              ...
              {pedidos.map((pedido) => (
                <tr key={pedido.idPedido}>
                  ...
                  <td className="actions-cell">
                    <button onClick={() => handleVerDetalle(pedido)} className="btn-view">Ver Detalle</button>
                    <button onClick={() => handleDownloadPDF(pedido.idPedido)} className="btn-icon">
                      <FaFilePdf size={20} />
                    </button>
                  </td>
                </tr>
          ...
          <div className="pagination-controls">
            ...
            <span>Página {currentPage} de {Math.ceil(totalPedidos / pedidosPerPage)}</span>
            <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === Math.ceil(totalPedidos / pedidosPerPage)}>
              Siguiente
            </button>
          </div>
          </>
